...
def makeBlendshapes():
    ...
    for i,s in enumerate(shapes): #loop through blendshape list
        ...
        if i==0: #first loop
            blendShape = cmds.blendShape(newShape,baseBlend[0],w=(i, 1))[0] #make blendshape node and connect first target
        else: #remaining loop
            cmds.blendShape( blendShape, edit=True, t=(baseBlend[0],i, newShape, 1.0),w=(i, 1) ) #connect remaining blendshapes
